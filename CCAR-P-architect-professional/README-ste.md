...
> **Note:** This edition follows the ASD-STE100 Simplified Technical English writing rules. Simple present tense, active voice, one word = one meaning, sentences of 20–25 words or less. Technical names (Claude, MCP, prompting, caching, effort, RAG, HITL, CLAUDE.md) are exceptions and stay as written.

**Exam code:** CCAR-P
**Credential:** Claude Certified Architect – Professional
**Pack type:** Original exam-prep notes (not official course content)
**Depth:** **Deepened for PRIMARY study**. Coverage includes architecture decision scenarios, trade-off tables, failure-mode analysis, production checklists, and expanded Q&A (25–40+ per domain file). The pack is a main study source. Use it with public Anthropic docs and the official exam guide (when you have access).
**Audience:** Solution architects who design production Claude systems
**User domains in this pack:** **Five** condensed files (maps onto official seven)
**Offline reading tip:** Each file is plain Markdown. Convert with any MD→EPUB tool if you want e-reader formatting. Keep headings H1–H3 for navigation.
...
| --- | --- | --- |
| Exam blueprint domains & weights | **Official** — Exam Guide v1.0 (effective July 2026). Download it from the official Pearson VUE Anthropic program page. | All weights and mechanics below come directly from it. We cross-checked them (2026-08-23) against the public Pearson VUE Anthropic program page. |
| Exam registration | Official portal | Registration and proctoring run through the official exam portal (Pearson VUE delivery). Check current requirements before you book. |
| Official gated prep course | Not used | This pack is original. It uses public sources only. |
...
| claude.ai / Claude API / Claude Code docs | **Public** (product features may require accounts) | Use these for hands-on practice. |
...
**Disclaimer:** This pack is an independent study aid. It is not affiliated with Anthropic. It is not endorsed by Anthropic. It does not derive from official course materials. Exam mechanics and domain weights can change. Check them against the official guide before exam day.
...
| --- | --- |
| Questions | **63** items |
| Time | **120 minutes** (about 1.9 min/question average) |
...
| Prerequisites | None mandatory. **Production Claude architecture experience is strongly recommended.** CCAR-F is helpful but not required. |

**Scoring note:** Scaled scoring means 720 is **not** a fixed % correct. Multiple-response items usually need **all** correct selections. Do not assume partial credit.
...
Read this information: Each domain file includes: disclaimer · overview · key map. Read this information: · deep notes · architecture decision scenarios · trade-off tables. Read this information: · failure-mode analysis · decision trees · exam traps ·. Read this information: **25–40+ Q&A** · production checklists · glossary. Deepened for primary study use.
...
| Addition | Where |
| --- | --- |
| **CSP / delivery-route governance** — public-safe decision table (Direct Anthropic API vs Amazon Bedrock vs Google Vertex AI vs Azure/Microsoft Foundry). Identity/auth, residency, logging/retention and ZDR-as-verify themes. Procurement/BAA/FedRAMP themes, model-ID differences, constraint-elimination vs guesses, plus Q&A. | `02` Part **D5** (pointer from `01` deployment surfaces) |
|**Model judges vs code judges + calibration**. When deterministic assertions win, when rubric judges are needed, human-gold calibration, thresholds/rollback **before** you look at experiment data, mini worksheet plus Q&A.|`02` Part **M7** (pointer from Part E3). |
...
3. **Week 3:** File 02 Evaluation half + File 03 safety/governance. Run the failure-mode tables.
4. **Week 4:** File 04 stakeholders/lifecycle/change/value (flag partner-only GTM limits) + File 05 enablement. Mixed scenarios.
5. **Weeks 5–6:** Timed 63Q/120m sets. Repeat weak areas. Write ADRs from the scenario banks without notes.
...
*This pack now serves as a primary study source across five user domain files (original notes only). It includes **P0 gap closures** (CSP routes, judge calibration, review-routing). Keep the 5↔7 domain mapping above. Check exam mechanics against the current official CCAR-P Exam Guide before the exam.*
