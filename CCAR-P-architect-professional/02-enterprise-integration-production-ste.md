---
title: 02 — Enterprise Integration & Production — Simplified Technical English
...
**Combined exam weight covered by this file:** ~**35%** (the largest technical share)

---

## Disclaimer

These are original study notes. They use **public** Anthropic docs (Messages API, tool use, MCP, RAG/citations, prompt caching, observability patterns). They also use published CCAR-P blueprint summaries.

Integration details and API surfaces change. Check the current platform documentation.

This pack is independent. It is not affiliated with Anthropic.

> **Note:** This edition follows the ASD-STE100 writing rules: simple present tense, active voice, one word = one meaning, sentences of 20–25 words or less. Technical names are exceptions and stay as written. They include Claude, MCP, Bedrock, Vertex, Foundry, Message Batches, prompting, caching, RAG, eval, p95, TTFT, SLA, IAM, SKU, canary, and golden set. Model IDs, routes, and API surfaces change. Learn the decision rules. Check the current docs before the exam.

---

## Overview

Solution Design selects the *shape*. Integration makes that design *real* in the enterprise. Evaluation proves that the system *works*.

CCAR-P gives Integration the highest weight (**19%**). Architects fail in production on authZ, retrieval quality, tool boundaries, latency, and observability. They do not fail on clever prose.

This file covers:

1. Tools, MCP, agents, and enterprise connectors
2. RAG / retrieval architecture and citations
3. AuthN/Z, least privilege, network and data paths
...
Developer enablement and Claude Code org rollout live in **`05`**. Safety and governance deep notes live in **`03`**. Entry-point model notes live in **`01`**. This file owns the enterprise **route-selection** decision table.
...
| --- | --- | --- |
| Select MCP vs REST vs CLI vs A2A | Integration 19% | Match transport to the trust boundary and the ops model |
| Design RAG chunk/index/retrieve/cite | Integration 19% | Match corpus and query patterns |
...
| Diagnose hallucinations / prompt fails | Evaluation 16% | Find the root cause. Then select the fix lever |
| Observability & optimization loops | Evaluation 16% | Logs → regressions → improve |
| Select CSP / delivery route under constraints | Integration 19% | Auth, residency, logging, procurement—not informal preference |
| Model judge vs code judge + calibration | Evaluation 16% | Deterministic first. Calibrate before you trust |
...
**Contract reminder (public tool-use model):** Claude never executes your tools by itself. It emits `tool_use`. **Your runtime** (or Anthropic for server tools) executes the tool. You return `tool_result`. You design that loop: timeouts, retries, idempotency, and privilege.

### A2. Client tools vs server tools

| Type | Who runs code? | Examples (illustrative) | Architect implication |
| --- | --- | --- | --- |
| Client / user-defined | Your app | CRM lookup, ticketing, internal SQL | You own authZ, logging, and sandboxing |
| Anthropic server tools | Anthropic | Web search/fetch, code execution, tool search (per docs) | Enable with intent. Know what data leaves your boundary |
...
MCP is an **open standard**. It connects assistants and agents to tools and data. Public docs emphasize:

- Local process, HTTP/SSE/streamable HTTP, and in-process transports. The ecosystem changes. Check current MCP and Claude Code docs.
- Servers expose tools, resources, and prompts.
- **Trust model:** Treat third-party servers as supply-chain risk. Prompt injection through fetched content is real.
- Large tool sets need **tool search** or deferred loading. Then definitions do not use too much of the context window.
- Channels and events (where supported) push external signals into sessions.
...
| You want reusable connectors across Claude Code and apps | Ultra-custom latency path |
| Org wants a connector catalog | Protocol overhead is not worth it |
| Partners already ship MCP servers | You must stay inside a locked-down RPC fabric |

### A4. Agent-to-agent and orchestration buses

Multi-agent systems need an explicit **handoff protocol**. That protocol has a message schema, a shared memory policy, and permission boundaries. Prefer:

- A clear supervisor or workflow that owns side effects
- Per-agent credentials
- Trace IDs across hops

Do not use many free-form agents that send many messages without budgets.

### A5. CLI and automation surfaces

Command-line tools (including Claude Code CLI in eng workflows—see file 05) fit CI, ops, and developer loops. For customer-facing products, prefer service APIs with authZ.

Exam scenarios: Select CLI for **builder productivity**. Select APIs for **product runtime**. The exception is when the product *is* a developer tool.
...
| --- | --- | --- |
| Policies with numbered sections | Section-aware chunks + headers | Keep clause IDs for citations |
| Code | Symbol/function-aware | Do not split a function in the middle without a reason |
| Tables/financials | Table-as-unit or row groups | Often need OCR structure |
| Chat logs | Turn windows with timestamps | PII scrubbing is mandatory |
| Slides | Per-slide + title | Low text density |

Overlap helps continuity. Too much overlap wastes tokens. It also duplicates noise.
...
Knowledge graphs help multi-hop “who owns X” questions. They also add ops cost. Justify the graph with eval gains.

### B4. Context packing & citations

Public Messages API patterns support **search result** content blocks and citations. Answers can then name their sources. Architect rules:

- Return source id/title/uri with chunks
- Require citations in the output contract when factual grounding matters
- If retrieval returns nothing relevant, **refuse or ask**. Do not invent policy
...
- Use tenant-isolated indexes or hard filters. Never “prompt the model to use only tenant A”
...
### B6. RAG failure modes (high exam value)

1. Wrong chunk boundaries → missed clauses
2. Embedding drift after a model change without a reindex plan
3. Cross-tenant leakage through a shared index
4. Prompt injection in retrieved HTML (“ignore policies…”)
5. Citation pretense: numbers that do not match sources
6. Over-retrieval that floods the model
7. Under-retrieval → hallucinations

### B7. Progressive discovery applied to RAG

Do not put the full corpus in the prompt. Use:

1. Metadata route (product=X)
2. Retrieve small k
3. If confidence is low, or the user asks for more depth, retrieve more. You can also browse hierarchical TOC tools
4. The agent may call `search` then `fetch_section` tools. Do not use one giant context
...
- [ ] AuthZ checked **in the tool server**, not only in the prompt
...
|Read/write split. |Read Research agent -only. The executor is gated. |
...
If you miss the budget, shrink context. Cache prefixes. Use a smaller model. Run retrieval in parallel. Or use async UX (“answer streaming + tools in the background”).
...
- Poison-tool detection (repeated failures → disable the tool, alert)
...
- **Prompt injection** through docs/tools → isolate untrusted content. Minimize write tools
- **Secrets** in vaults. Give short-lived tokens to tools
- **Egress controls** for fetch/MCP
- **Data paths:** what leaves the VPC to the Anthropic/cloud AI endpoint. ZDR/retention options per contract
- **Logging redaction** for prompts that contain PII
...
Architects do not select "Claude on X" because a presentation looks new. They remove routes that violate **identity, residency, logging/retention, procurement, and model-ID** constraints. Then they select among the remaining routes on ops fit and feature lag.

Public surfaces (verify current docs. Offerings change):

| Route | What it is (public framing) |
| --- | --- |
| **Direct Anthropic API** | First-party Messages API (`platform.claude.com`). Anthropic operates inference |
| **Amazon Bedrock** | Claude through AWS Bedrock. AWS-managed boundary / regional controls (Messages and/or legacy Invoke/Converse paths per docs) |
| **Google Vertex AI** | Claude through Google Cloud Vertex. GCP IAM + regional endpoints |
...
| **Claude Platform on AWS** *(related)* | Anthropic-operated Claude on an AWS Marketplace-style path. This is **not** the same as Bedrock. Residency and inference pinning differ. Do not mix them in exam stems |

### D5.1 Public-safe decision table

Treat cells as **themes to verify in the live contract/docs**. They are not eternal guarantees. ZDR and BAAs are **enterprise options**. Confirm entitlement before you design as if they exist.

| Dimension | Direct Anthropic API | Amazon Bedrock | Google Vertex AI | Azure / Microsoft Foundry |
| --- | --- | --- | --- | --- |
| **Identity / auth** | API keys / org and workspace controls. Your app still owns user→tool identity | **AWS IAM** (roles, SCPs, PrivateLink patterns). CloudTrail-friendly | **GCP IAM** + Vertex permissions. Org policies | **Entra ID / Azure RBAC** + Foundry project controls |
...
| **Logging / retention posture** | Anthropic retention and abuse-monitoring terms. **ZDR** may be available as an enterprise option—**verify** | AWS/Bedrock data-protection docs plus your account logging govern it. Do not assume Anthropic ZDR wording maps 1:1 | Governed by **Google Cloud / Vertex** data processing plus your logging sinks | Anthropic processor terms + Azure-side retention. Safety-flagged content paths may differ by hosting—**read current Foundry Claude docs** |
| **Procurement / BAA / FedRAMP themes** | Direct MSA / DPA with Anthropic. BAA/FedRAMP only if **your** deal and offering support them | Buy through **AWS Marketplace / Bedrock**. Use existing AWS commit, AWS compliance artifacts, and org SCPs. Still map PHI/FedRAMP to the **specific** Bedrock Claude offering | Buy through **GCP**. Reuse GCP commit and org policy. Same rule: an artifact is not an automatic BAA | Buy through **Azure Marketplace / Foundry**. Reuse Microsoft EA. Confirm HIPAA/FedRAMP against the **deployment type** you actually provision |
| **Model ID differences** | Anthropic model IDs (e.g. dated `claude-…` strings in Messages API) | Bedrock **model IDs / inference profile IDs / ARNs** (and Messages path IDs where enabled). These are **not** a copy-paste of first-party IDs | Vertex publisher model IDs / versions—**platform-specific** | Foundry **deployment names** + model versions (hosting option encoded). App config must not hardcode another CSP’s ID |
| **Feature / API lag themes** | Often **earliest** access to new Messages features, betas, server tools | May lag or expose through Bedrock-shaped APIs. Check parity before you promise a feature | Similar parity/lag story on GCP | Parity depends on Azure-hosted vs Anthropic-hosted deployment. Feature matrices differ |
| **When architects select this** | Max feature velocity. Simple SaaS. Residency solvable through `inference_geo` + contract. Team already on Anthropic billing | AWS-centric estate. IAM/PrivateLink mandates. Residency through AWS regions. Procurement must stay inside AWS | GCP-centric estate. Vertex is the approved AI plane. Org policy blocks other egress | Azure-centric estate. Entra + Foundry is the approved AI plane. Need US Data Zone or Azure-hosted boundary |

**Always true on every route:** your **app** still owns tenancy authZ, tool least privilege, prompt/log redaction, and eval gates. A CSP switch does not fix “authZ in the prompt.”
...
2. Strike any route that cannot meet a hard constraint (verify assumptions—do not invent FedRAMP).
...
**Informal preference (fail):** “Everyone uses Bedrock.” / “Direct API is always fastest to value so pick it.” / “Foundry because we like Copilot.”
...
- Pin **versioned** IDs in prod. Control upgrades through flags + eval gates.
...
- Abuse-monitoring / safety-flagged egress may still exist under provider terms. Design as if “ZDR ≠ invisible to all safety systems” until counsel/docs say otherwise.
- Cloud routes: your CloudTrail/Cloud Logging/Diagnostic logs may retain **metadata and sometimes payloads** you configured. Architect the sink. Do not architect only the model vendor story.

### D5.5 Exam traps — delivery routes

1. Picking a CSP for brand preference while you ignore residency in the stem.
...
8. Believing CSP choice alone satisfies GDPR/HIPAA. You still need data maps and app controls.
...
**A.** Azure/Foundry with a **US Data Zone** (or equivalent US-pinned) deployment type per current docs. Do not use Direct API global by default.

**Q74.** Select TWO reasons to prefer Bedrock over Direct API in an AWS-only regulated estate.
**A.** Reuse **IAM/SCPs/PrivateLink** patterns. Keep procurement/compliance evidence inside the **AWS** boundary the org already operates.
...
**Q77.** Product needs a brand-new Messages API beta next week. The estate is multi-cloud with no hard residency pin. Which route often wins and why?
**A.** **Direct Anthropic API**—typically earliest feature access. Confirm beta entitlement. Still document residency/logging.

**Q78.** FedRAMP-like stem lists “authorized cloud AI service only.” What do you do first?
**A.** **Constraint-eliminate** routes using the customer’s authorized service list / boundary docs. Then design inside the survivor (logging, change control). Do not “add a safer prompt.”

**Q79.** Vertex vs Direct API when GCP org policy blocks egress to non-Google AI endpoints?
**A.** **Vertex**—Direct API may be unreachable or noncompliant under that org policy. Model quality does not change this.

**Q80.** Pick-for-preference vs constraint-elimination—give one exam tell for each.
**A.** Informal preference: the answer cites popularity/speed alone. Constraint-elimination: the answer cites a stem constraint that **disqualifies** other routes before you optimize.
...
Hybrid is normal. Automations gate every PR. Humans sample weekly. Humans also review high-severity slices.

**P0 expansion:** see **Part M7 — Model judges vs code judges + calibration**. That part says when deterministic graders beat LLM-as-judge. It also says how you calibrate against human gold. Freeze thresholds **before** you look at experiment results.

### E4. Experimentation discipline

- Change **one** major variable at a time when you diagnose
- Record model ID, prompt hash, index version, tool versions
- Use statistical sense on A/B. Do not declare a winner on 20 chats
...
|Inconsistent quality. |Underspecified policy/contract. |Reject Clarify contract, schema + few-shot anchors (sampling knobs on current tiers).|
...
Store raw prompts with care (PII, retention). Prefer pointers + redacted snippets for long-term storage.
...
Workflow front door routes: FAQ RAG vs account-tool path vs human. Account tools use scoped OAuth. Writes require confirm. Traces go to the warehouse for weekly rubric grading.

### F3. MCP-connected builder environment

Claude Code + approved MCP marketplace (file 05) for eng productivity. Keep this **separate** from customer runtime credentials. Never reuse prod write tokens in IDE agents.
...
1. Put the entire KB into context instead of RAG.
2. AuthZ “in the prompt only.”
3. Agent with unbounded tools and no max-turn budget.
4. Evaluating only informal checks / selected demo cases.
5. Optimizing cost before you meet accuracy/safety bars.
6. Ignoring tenant isolation in a shared vector DB.
...
13. Picking Direct API / Bedrock / Vertex / Foundry for brand preference instead of constraint elimination.
14. Assuming ZDR or BAA exists on a route without verifying entitlement.
15. Freezing eval pass thresholds only after you look at A/B results.
...
**A.** Large tool catalogs consume context. Search loads only the tools you need.
...
**A.** Whether cited sources actually support the claims you make.
...
**A.** It reduces cost/latency for stable system prompts, tool defs, and policy prefixes.
...
**A.** Prevent cross-tenant retrieval leakage. Prompt instructions alone are not sufficient.
...
**A.** Cap mapper outputs, use hierarchical reduce, or summarize intermediate notes with schema.
...
**A.** Developer/ops automation. Usually not the end-customer product path unless the product is a CLI.
...
**Q21.** How do you stop false citations?
...
**A.** Start with few tools/context. Fetch more through search/browse as needed.

**Q23.** Why separate builder MCP credentials from runtime product credentials?
**A.** Scope of damage—IDE agents must not hold prod write power.
...
| Circuit breaker | Stop calling failing dependencies for a time |
...
| Judge calibration | Measuring LLM-as-judge agreement vs human gold before you trust scores |
| Pre-registration | Fixing metrics/thresholds/rollback before you see experiment outcomes |
...
Enterprise integrations fail when the model path “becomes the user” with one shared token that has all privileges. Prefer:

1. User authenticates to your app (OIDC/SAML).
2. The app gets downstream tokens limited to that user's scopes. (OAuth on-behalf-of, token exchange).
3. Tools call APIs with those tokens.
4. Audit logs attribute actions to the human + service principal.

Service accounts are for batch jobs with separate controls. Do not use them as a shortcut for interactive agents.

### G2. Network patterns

| Pattern | Use |
| --- | --- |
| Public Claude API egress | Simplest. Control through proxy/DLP |
...
If an interactive user waits, use tight tool timeouts and streaming text. Defer slow tools to “still working” jobs. Back-office automation uses queue workers, Batch API, and longer tool budgets. Do not force both into one latency class.
...
Many “model problems” are retrieval problems. Fix the index before you swap to a giant model.

### G6. Online evaluation & feedback

Capture implicit signals: edits, regenerations, thumbs, escalation. Route low-confidence or high-severity traffic to human labels. Feed these labels into the regression sets weekly. Without this, offline gold sets decay.
...
Pin model versions consistently. Replicate indexes with conflict rules. Use sticky sessions for conversation state. Know cloud endpoint global vs regional routing trade-offs from public docs. Fail closed on auth. Fail soft on optional enrichment tools.

### G9. Additional scenarios

**Scenario:** Manufacturing firm wants Claude to query SCADA historian.
**Answer shape:** Never raw plant network from a SaaS agent. Use a DMZ read API, allowlisted metrics, and human approval for actuation. Prefer no write tools at first. RAG over manuals + read tools for telemetry summaries.
...
**Answer shape:** Inspect tool definition token share. Enable tool search. Shrink allowlists. Cache system+tool prefixes. Add max turns. Fix loops that call search repeatedly.

### G10. Extended Q&A (29–36)

**Q29.** Why is on-behalf-of token exchange better than a shared token with all privileges?
**A.** It preserves user-scoped authZ and attributable audit trails.
...
**A.** Whether retrieved evidence actually influences the answer, or only wastes tokens.

**Q32.** How do you test authZ for tools?
**A.** Negative tests: identities that lack roles must be denied by the tool server.

**Q33.** Why might cache hit rate collapse after a “small prompt tweak”?
**A.** Prefix bytes changed (or tool order/thinking config). That invalidates breakpoints.

**Q34.** What is fail-soft for optional enrichment?
**A.** Return the core answer if an optional tool/RAG times out, with explicit partial-status. Do not return a hard 500.
...
**A.** Who is allowed, how is it audited, and what is the scope of damage if prompt-injected?

---

## Extended checklist

- [ ] I can describe OBO/token exchange vs tokens with all privileges
...
| Fail-soft | Degrade in a controlled way when non-critical deps fail |
...
- Replays return the original result without charging twice / double-shipping.
...
| Server session store | Truncation/compaction control | Consistency and retention duties |
| Summary + recent window | Token efficient | Summary drift |

Pair with compaction features from the platform when sessions run long (see file 01). Always define retention aligned to legal holds.
...
Rollback is a flag flip. It is not a wish without a control.
...
Page on-call for availability/latency/safety. Review quality in business hours unless severity is high.

### H5. Partner / vendor MCP supply chain

Before you enable a vendor MCP server:

1. Threat model (what data it can read/write)
2. Auth method (OAuth scopes minimized)
3. Changelog and version pin policy
4. Sandbox trial on synthetic data
5. Legal review if data leaves the boundary
...
Who labels? With what rubric? Calibration sessions? Dual annotation on hard slices? Do you track inter-annotator agreement? Architects who ignore labeling quality ship false confidence.
...
15. Eval only single-turn when the product is agentic.
16. Enabling vendor MCP without scope review.
17. Quality paging that tires the on-call staff with non-actionable noise.

### H11. Q&A 37–45

**Q37.** Why can Claude not "remember" a refund across retries by itself?
...
**A.** Typically Data + Security accountable. Eng is responsible for implementation—per RACI.

**Q42.** Why pin MCP server versions?
**A.** Prevent surprise tool schema/behavior changes that break agents/evals.

**Q43.** What SLO belongs on a safety probe suite?
**A.** Harmless-response / attack-blocked rate with alert thresholds.

**Q44.** Stream tokens to the UI, but when do you commit a database write?
**A.** Only after a validated completed tool flow—not on partial plan text.

**Q45.** What is a labeling calibration session?
**A.** Annotators align on the rubric with examples. This raises agreement before you score.
...
Place an AI gateway for: auth, quotas, prompt size limits, PII redaction hooks, model routing, and centralized traces. Application teams call the gateway. They do not call raw provider keys. Rotate keys centrally. Attribute cost by team tags.

### I2. Event-driven tool results

Long tools (report generation) should acknowledge immediately. Then they push completion events. Resume the agent/workflow with `tool_result` when ready. Avoid holding HTTP requests open for minutes without a product reason.
...
4. Cut traffic.
5. Keep the old index until the rollback window ends.
...
**A.** Async job + event/callback resume. Do not use a single blocking HTTP call without design.

**Q48.** What is dual-write during reindex?
**A.** Writing embeddings to old and new indexes during migration so you can cut over or roll back.
...
**A.** Retrieval quality shifts with no reproducible version. That breaks evals and incidents.
...
**A.** Memory is stale or can be invented. Systems of record are authoritative.
...
Integration (19%) asks whether you can connect Claude to enterprise reality. You must not leak tenancy. You must not run unbounded agents. You must not work without quality telemetry. Evaluation (16%) asks whether you can prove behavior with datasets, experiments, and telemetry. You must also optimize in the right order. If you remember only five slogans, remember these:
...
Before exam day, confirm you can whiteboard the following. Whiteboard hybrid RAG with tenant filters, and an MCP allowlist example. Whiteboard an agent with max-turn and dollar budgets. Whiteboard a CI eval gate and a cost-anomaly runbook. If any whiteboard feels unclear, reread the matching part above. Then redo two Q&As aloud.
...
| --- | --- | --- | --- |
| Direct Messages API + client tools | Product runtime with 1–N bespoke tools | App identity + per-tool server checks | Ad-hoc tools sprawl without a catalog |
...
| CLI / local automation | Builder and CI loops | Local creds / OIDC device flows | Using CLI secrets in the customer SaaS path |
| Agent-to-agent (A2A) | Clear role split across services | Per-agent credentials + hop audit | Excessive message traffic without budgets or owners |
| Server tools (Anthropic-hosted) | Web fetch/search/code where allowed | Understand egress and data handling | Enabling “because it is interesting” in regulated tenants |
| Event bus + workers | Long tools / human queues | Queue ACLs + idempotency keys | Dual-writes without correlation IDs |

**Exam heuristic:** Prefer **MCP** when reuse and discovery matter. Prefer **direct tools** when latency and ultra-custom control dominate. Prefer **A2A** only with a supervisor and schemas. Prefer **CLI** for builders, not end-customer prod paths (unless the product *is* a CLI).

### K2. Capability bloat analysis (exam favorite)

Symptoms of tool/agent bloat:
- Model picks the wrong tool among near-duplicates
...
- [ ] User identity propagated (not a shared service account with all privileges for all tenants)
- [ ] Tool execution checks **resource-level** authZ in *your* code—not “Claude promises in the prompt”
...
|Setting. |Accuracy ↑?|Latency ↑?|Notes. |
| --- | --- | --- | --- |
...
Justify each setting with **metric + budget**. If you max all knobs without measurement, you fail scenario questions.
...
| Citation pretense | Cite-but-unused checks | Require quote spans or ids that the answer uses |
| OCR garbage | Field validators | Human quarantine. Vision quality gates |
| Over-retrieve | Token and latency dashboards | Lower k. Pack better |
...
**Common exam error:** Optimizing BLEU-like n-gram scores for a tool-using agent. Prefer **task success** and **tool correctness**.
...
- Record negative results. Avoid uncontrolled repeated changes.
...
LLM-as-judge is a **measurement instrument**. It is not an informal scoring method. Architects select the grader the way they select a unit-test framework. Prefer **deterministic checks** when the contract is machine-checkable. Use **model judges** when quality is open-ended. **Calibrate** before the score drives ship/no-ship.
...
**Rule:** If a junior engineer could write a pytest without reading the essay, **do not** spend tokens on a judge.
...
|Nuanced faithfulness. |The platform supports Soft check that claims when citations are prose-y.|

Even then: combine with code gates (schema, cite-ID ⊆ retrieval). Then the judge never scores garbage JSON as “beautiful.”
...
**Common exam error:** “Judge score went up 8% after prompt tweak → ship.” Without calibration, you may have taught the judge to prefer long answers.

#### Thresholds & rollback — freeze BEFORE seeing experiment data

| Pre-register (write down first) | Why |
| --- | --- |
| Primary metric + direction | Stops metric shopping after results |
...
If you look at results and then select thresholds, that is p-hacking. Architects treat eval configs as controlled artifacts in git.
...
3. Changing success thresholds after you see the A/B dashboard.
...
**A.** Primary metric with threshold **and** rollback/guardrail criteria—written before you look at experiment outcomes.

**Q84.** Summaries graded only by exact string match keep failing good paraphrases. What do you do?
**A.** Keep code checks for must-include facts/citations. Add a **calibrated** rubric judge (or human sample) for paraphrase quality. Do not abandon factual assertions.

**Q85.** Judge scores rise after you switch the judge to the same model family as the generator. Concern?
**A.** **Shared bias / self-preference**. Re-calibrate vs humans. Consider a different judge model or pairwise protocol.

**Q86.** What is a sound rollback criterion example?
**A.** “If adversarial injection ASR > X% or citation-ID validation fail rate > Y% on canary, auto-disable flag.” Set this **before** the canary.

**Q87.** Offline gold looks great. Prod thumbs-down spike. First eval move?
**A.** Suspect **distribution shift**. Sample prod failures into a new slice. Do not only retune the judge prompt to match thumbs.

**Q88.** When are human panels still required even with a calibrated judge?
**A.** High-stakes / irreversible domains, novel failure modes, and periodic audit of the judge itself. Automation supplements accountability. It does not replace it.
...
AuthZ: refund tool capped. Amount comes from code, not a free-form model float without a check.

### N3. Observability missing after GA

Symptom: rising “wrong answer” tickets, no traces.
Fix: add correlation IDs. Store redacted prompts/tool I/O. Build a replay harness. Freeze a regression set from tickets.

### N4. Accuracy up, latency broken

You added critique loop + k=20 retrieve + opus-class.
Rollback to baseline. A/B each setting. Keep only levers with proven lift per +100ms.
...
| Conversation state loss | Amnesia mid-task | Durable state store. Do not rely on client only |
...
**Q56.** Select TWO symptoms of capability bloat. Option (a) is wrong tool selection. Option (b) is faster p95. Option (c) is huge tool defs in context. Option (d) is higher cache hit rate.
**A.** **(a)** and **(c)**.
...
**Reject:** Monolithic context of all policies. Shared credentials with all privileges. Agent with write access to HRIS on day one.
...
Select the minimal config that meets the quality bar on the stratified eval set.
...
*Largest technical file by blueprint weight. P0 gap closures: Part D5 (CSP routes), Part M7 (judges).*
